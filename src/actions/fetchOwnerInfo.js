...
export const fetchOwnerInfo = (id) => (dispatch) => {
    ...
    fetch(url, { 
      ...
      .then((data) => {
          console.log(data)
          return dispatch({ type: OWNER_LOAD_OWNER_INFO_SUCCESS, payload: {name: data.FirstName, surname: data.LastName} });
      })
      .catch((err) => dispatch({ type: OWNER_LOAD_OWNER_INFO_FAIL, payload: err }));
  };